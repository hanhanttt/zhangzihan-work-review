...
function renderFuturePlans(){$('#future-plan-grid').innerHTML=state.data.futurePlans.map((p,i)=>`<article class="future-plan-card"><div class="future-plan-number">0${i+1}</div><p class="future-plan-type">${p.type}</p><h3>${p.title}</h3><p class="future-plan-summary">${p.summary}</p>${p.link?`<a class="future-plan-link" href="${p.link}" target="_blank" rel="noopener noreferrer">${p.linkLabel||'查看相关资料'} ↗</a>`:''}${p.image?`<figure class="future-plan-visual"><img src="${p.image}" alt="${p.imageAlt||p.title}" loading="lazy"></figure>`:''}${p.basis?`<div class="future-plan-block plan-basis"><strong>方案依据</strong><p>${p.basis}</p></div>`:''}<div class="future-plan-block"><strong>协作与输入</strong><p>${p.input}</p></div><div class="future-plan-block"><strong>重点行动</strong><ul>${p.actions.map(x=>`<li>${x}</li>`).join('')}</ul></div><div class="future-plan-block"><strong>目标闭环</strong><p>${p.loop}</p></div>${p.channels?`<div class="channel-list">${p.channels.map(x=>`<span>${x}</span>`).join('')}</div>`:''}</article>`).join('')}
